...
def export_detailed_validation_excel(
    zone: str = "SE3",
    year: int = 2024,
    sol_profile_name: str = "south_lundby",
    sol_mw: float = 0.4,
    vind_mw: float = 1.6,
    baseload_mw: float = 0.2,
    efficiency: float = 0.90,
    filepath: Optional[str] = None,
) -> Path:
    """
    Export detailed validation Excel with formulas and input data.

    Creates workbook with 7 sheets for full traceability:
    1. Parametrar - Editable parameters
    2. Sol Profil - Solar profile input data (8760 rows)
    3. Vind Profil - Wind profile input data (8760 rows)
    4. Timberäkning - Hourly calculations with formulas
    5. Daglig Summering - Daily aggregation (366 rows)
    6. Kritiska Timmar - Top 100 deficit hours
    7. Sammanfattning - Summary with key metrics

    Args:
        zone: Electricity zone (SE1-SE4)
        year: Analysis year
        sol_profile_name: PVsyst profile name
        sol_mw: Solar installed capacity (MW)
        vind_mw: Wind installed capacity (MW)
        baseload_mw: Target baseload (MW)
    ...
    if filepath is None:
        filepath = DATA_DIR / "reports" / f"baseload_validation_{zone}_{year}.xlsx"
    else:
        filepath = Path(filepath)
    ...
    header_font = Font(bold=True, color='FFFFFF')
    header_fill = PatternFill('solid', fgColor='1B4F72')
    input_fill = PatternFill('solid', fgColor='FFF2CC')  # Light yellow for editable
    highlight_fill = PatternFill('solid', fgColor='E8F6F3')  # Light green
    warning_fill = PatternFill('solid', fgColor='FADBD8')  # Light red
    border = Border(
        left=Side(style='thin'),
        right=Side(style='thin'),
        top=Side(style='thin'),
    ...
    # ========================================
    # Sheet 1: Parametrar (editable)
    # ========================================
    ws_param = wb.active
    ws_param.title = "Parametrar"

    ws_param['A1'] = "BASELOAD PPA VALIDERING"
    ws_param['A1'].font = Font(bold=True, size=14)
    ws_param.merge_cells('A1:C1')

    ws_param['A3'] = "REDIGERBARA PARAMETRAR"
    ws_param['A3'].font = Font(bold=True)
    ws_param['A3'].fill = header_fill
    ws_param['A3'].font = header_font

    param_rows = [
        ("Baseload (MW)", baseload_mw, "B4"),
        ("Batterieffektivitet", efficiency, "B5"),
        ("Sol installerat (MW)", sol_mw, "B6"),
        ("Vind installerat (MW)", vind_mw, "B7"),
    ]

    for i, (label, value, cell) in enumerate(param_rows):
        row = 4 + i
        ws_param[f'A{row}'] = label
        ws_param[cell] = value
        ws_param[cell].fill = input_fill
        ws_param[cell].number_format = '0.000' if 'MW' in label else '0.00'

    # Fixed parameters
    ws_param['A9'] = "FASTA PARAMETRAR"
    ws_param['A9'].font = Font(bold=True)

    fixed_params = [
        ("Zon", zone),
        ("År", year),
        ("Solprofil", sol_profile_name),
        ("Timmar", len(timeseries)),
    ]

    for i, (label, value) in enumerate(fixed_params):
        row = 10 + i
        ws_param[f'A{row}'] = label
        ws_param[f'B{row}'] = value

    # Calculated metrics (formulas)
    ws_param['A15'] = "BERÄKNADE NYCKELTAL"
    ws_param['A15'].font = Font(bold=True)
    ws_param['A15'].fill = header_fill
    ws_param['A15'].font = header_font

    calc_metrics = [
        ("Total produktion (MWh)", "=SUM(Timberäkning!D:D)", "0.0"),
        ("Baseload-behov (MWh)", "=B4*B13", "0.0"),
        ("Total deficit (MWh)", "=SUM(Timberäkning!G:G)", "0.00"),
        ("Total surplus (MWh)", "=SUM(Timberäkning!H:H)", "0.00"),
        ("Deficit-timmar", "=COUNTIF(Timberäkning!G:G,\">0\")", "0"),
        ("Surplus-timmar", "=COUNTIF(Timberäkning!H:H,\">0\")", "0"),
        ("Max deficit (MW)", "=MAX(Timberäkning!G:G)", "0.000"),
        ("Max batteribehov (MWh)", "=MAX(Timberäkning!K:K)", "0.00"),
        ("Batteriduration (h)", "=B23/B4", "0.0"),
    ]

    for i, (label, formula, fmt) in enumerate(calc_metrics):
        row = 16 + i
        ws_param[f'A{row}'] = label
        ws_param[f'B{row}'] = formula
        ws_param[f'B{row}'].number_format = fmt
        ws_param[f'B{row}'].fill = highlight_fill
    ...
    # Sheet 2: Sol Profil
    # ========================================
    ws_sol = wb.create_sheet("Sol Profil")

    sol_headers = ["Timestamp", "Månad", "Dag", "Timme", "Norm (0-1)", "MW"]
    for col, header in enumerate(sol_headers, 1):
        cell = ws_sol.cell(row=1, column=col, value=header)
        cell.font = header_font
        cell.fill = header_fill

    for row_idx, row_data in enumerate(timeseries, start=2):
        ts = row_data["timestamp"]
        key = (ts.month, ts.day, ts.hour)
        norm_value = sol_profile.get(key, 0.0)

        ws_sol.cell(row=row_idx, column=1, value=ts.strftime("%Y-%m-%d %H:00"))
        ws_sol.cell(row=row_idx, column=2, value=ts.month)
        ws_sol.cell(row=row_idx, column=3, value=ts.day)
        ws_sol.cell(row=row_idx, column=4, value=ts.hour)
        ws_sol.cell(row=row_idx, column=5, value=norm_value).number_format = '0.0000'
        # MW column with formula referencing parameter
        ws_sol.cell(row=row_idx, column=6, value=f"=E{row_idx}*Parametrar!$B$6")

    ws_sol.freeze_panes = 'A2'

    # ========================================
    # Sheet 3: Vind Profil
    # ========================================
    ws_vind = wb.create_sheet("Vind Profil")

    vind_headers = ["Timestamp", "Månad", "Dag", "Timme", "CF (0-1)", "MW"]
    for col, header in enumerate(vind_headers, 1):
        cell = ws_vind.cell(row=1, column=col, value=header)
        cell.font = header_font
        cell.fill = header_fill
    ...
    for row_idx, row_data in enumerate(timeseries, start=2):
        ts = row_data["timestamp"]
        key = (ts.month, ts.day, ts.hour)
        cf_value = vind_profile.get(key, 0.0)

        ws_vind.cell(row=row_idx, column=1, value=ts.strftime("%Y-%m-%d %H:00"))
        ws_vind.cell(row=row_idx, column=2, value=ts.month)
        ws_vind.cell(row=row_idx, column=3, value=ts.day)
        ws_vind.cell(row=row_idx, column=4, value=ts.hour)
        ws_vind.cell(row=row_idx, column=5, value=cf_value).number_format = '0.0000'
        # MW column with formula referencing parameter
        ws_vind.cell(row=row_idx, column=6, value=f"=E{row_idx}*Parametrar!$B$7")

    ws_vind.freeze_panes = 'A2'

    # ========================================
    # Sheet 4: Timberäkning (formulas)
    # ========================================
    ws_calc = wb.create_sheet("Timberäkning")

    calc_headers = [
        "Timestamp", "Sol (MW)", "Vind (MW)", "Total (MW)", "Baseload (MW)",
        "Diff (MW)", "Deficit (MW)", "Surplus (MW)", "Net Flow (MWh)",
        "Cum Sum", "SOC (MWh)"
    ]
    ...
    for row_idx, row_data in enumerate(timeseries, start=2):
        ...
        # A: Timestamp
        ws_calc.cell(row=excel_row, column=1, value=ts.strftime("%Y-%m-%d %H:00"))

        # B: Sol (MW) - reference to Sol Profil
        ws_calc.cell(row=excel_row, column=2, value=f"='Sol Profil'!F{excel_row}")

        # C: Vind (MW) - reference to Vind Profil
        ws_calc.cell(row=excel_row, column=3, value=f"='Vind Profil'!F{excel_row}")

        # D: Total (MW) = Sol + Vind
        ws_calc.cell(row=excel_row, column=4, value=f"=B{excel_row}+C{excel_row}")

        # E: Baseload (MW) - reference to parameter
        ws_calc.cell(row=excel_row, column=5, value="=Parametrar!$B$4")

        # F: Diff (MW) = Total - Baseload
        ws_calc.cell(row=excel_row, column=6, value=f"=D{excel_row}-E{excel_row}")

        # G: Deficit (MW) = MAX(0, -Diff)
        ws_calc.cell(row=excel_row, column=7, value=f"=MAX(0,-F{excel_row})")

        # H: Surplus (MW) = MAX(0, Diff)
        ws_calc.cell(row=excel_row, column=8, value=f"=MAX(0,F{excel_row})")

        # I: Net Flow (MWh) = IF surplus, apply efficiency; if deficit, full amount
        ws_calc.cell(row=excel_row, column=9,
                     value=f"=IF(F{excel_row}>0,H{excel_row}*Parametrar!$B$5,-G{excel_row})")

        # J: Cum Sum - running total
        if excel_row == 2:
            ws_calc.cell(row=excel_row, column=10, value=f"=I{excel_row}")
        else:
            ws_calc.cell(row=excel_row, column=10, value=f"=J{excel_row-1}+I{excel_row}")

        # K: SOC (MWh) = battery level needed
        # This is complex - simplified to show max drawdown from cumsum peak
        ws_calc.cell(row=excel_row, column=11,
                     value=f"=MAX($J$2:J{excel_row})-J{excel_row}")

    # Set number formats
    for col in range(2, 12):
        for row in range(2, len(timeseries) + 2):
            ws_calc.cell(row=row, column=col).number_format = '0.000'

    ws_calc.freeze_panes = 'A2'

    # Column widths
    for col in range(1, 12):
        ws_calc.column_dimensions[get_column_letter(col)].width = 12

    # ========================================
    # Sheet 5: Daglig Summering
    # ========================================
    ws_daily = wb.create_sheet("Daglig Summering")

    daily_headers = [
        "Datum", "Sol (MWh)", "Vind (MWh)", "Total (MWh)", "Baseload (MWh)",
        "Deficit (MWh)", "Surplus (MWh)", "Max SOC (MWh)", "Status"
    ]
    ...
    for i, row_data in enumerate(timeseries):
        date_str = row_data["timestamp"].strftime("%Y-%m-%d")
        daily_data[date_str].append(i + 2)  # Excel row number (1-indexed, +1 for header)

    for row_idx, (date_str, hour_rows) in enumerate(sorted(daily_data.items()), start=2):
        first_row = min(hour_rows)
        last_row = max(hour_rows)

        ws_daily.cell(row=row_idx, column=1, value=date_str)

        # B: Sol (MWh) - sum of hourly
        ws_daily.cell(row=row_idx, column=2,
                      value=f"=SUM(Timberäkning!B{first_row}:B{last_row})")

        # C: Vind (MWh)
        ws_daily.cell(row=row_idx, column=3,
                      value=f"=SUM(Timberäkning!C{first_row}:C{last_row})")

        # D: Total (MWh)
        ws_daily.cell(row=row_idx, column=4,
                      value=f"=SUM(Timberäkning!D{first_row}:D{last_row})")

        # E: Baseload (MWh) = baseload × 24h
        ws_daily.cell(row=row_idx, column=5, value="=Parametrar!$B$4*24")

        # F: Deficit (MWh)
        ws_daily.cell(row=row_idx, column=6,
                      value=f"=SUM(Timberäkning!G{first_row}:G{last_row})")

        # G: Surplus (MWh)
        ws_daily.cell(row=row_idx, column=7,
                      value=f"=SUM(Timberäkning!H{first_row}:H{last_row})")

        # H: Max SOC during day
        ws_daily.cell(row=row_idx, column=8,
                      value=f"=MAX(Timberäkning!K{first_row}:K{last_row})")

        # I: Status
        ws_daily.cell(row=row_idx, column=9,
                      value=f'=IF(F{row_idx}>0,"DEFICIT","OK")')

    # Set number formats
    for col in range(2, 9):
        for row in range(2, len(daily_data) + 2):
            ws_daily.cell(row=row, column=col).number_format = '0.00'

    ws_daily.freeze_panes = 'A2'

    # Column widths
    for col in range(1, 10):
        ws_daily.column_dimensions[get_column_letter(col)].width = 13

    # ========================================
    # Sheet 6: Kritiska Timmar (top 100 deficit hours)
    # ========================================
    ws_critical = wb.create_sheet("Kritiska Timmar")

    crit_headers = [
        "Rank", "Timestamp", "Sol (MW)", "Vind (MW)", "Total (MW)",
        "Baseload (MW)", "Deficit (MW)", "Cum Sum"
    ]

    for col, header in enumerate(crit_headers, 1):
        cell = ws_critical.cell(row=1, column=col, value=header)
        cell.font = header_font
        cell.fill = header_fill

    # Calculate battery requirement to get actual values
    battery_info = calculate_battery_requirement(timeseries, baseload_mw, efficiency)

    # Find top 100 deficit hours
    deficit_hours = []
    cumsum = 0.0
    for i, row_data in enumerate(timeseries):
        diff = row_data["total_mw"] - baseload_mw
        deficit = max(0, -diff)
        surplus = max(0, diff)
        net_flow = surplus * efficiency if diff > 0 else -deficit
        cumsum += net_flow

        if deficit > 0:
            deficit_hours.append({
                "timestamp": row_data["timestamp"],
                "sol_mw": row_data["sol_mw"],
                "vind_mw": row_data["vind_mw"],
                "total_mw": row_data["total_mw"],
                "deficit_mw": deficit,
                "cumsum": cumsum,
            })

    # Sort by deficit (worst first) and take top 100
    deficit_hours.sort(key=lambda x: -x["deficit_mw"])
    top_100 = deficit_hours[:100]

    for row_idx, hour in enumerate(top_100, start=2):
        ws_critical.cell(row=row_idx, column=1, value=row_idx - 1)  # Rank
        ws_critical.cell(row=row_idx, column=2, value=hour["timestamp"].strftime("%Y-%m-%d %H:00"))
        ws_critical.cell(row=row_idx, column=3, value=hour["sol_mw"]).number_format = '0.000'
        ws_critical.cell(row=row_idx, column=4, value=hour["vind_mw"]).number_format = '0.000'
        ws_critical.cell(row=row_idx, column=5, value=hour["total_mw"]).number_format = '0.000'
        ws_critical.cell(row=row_idx, column=6, value=baseload_mw).number_format = '0.000'
        ws_critical.cell(row=row_idx, column=7, value=hour["deficit_mw"]).number_format = '0.000'
        ws_critical.cell(row=row_idx, column=8, value=hour["cumsum"]).number_format = '0.00'

        # Highlight row
        for col in range(1, 9):
            ws_critical.cell(row=row_idx, column=col).fill = warning_fill

    ws_critical.freeze_panes = 'A2'

    # Column widths
    for col in range(1, 9):
        ws_critical.column_dimensions[get_column_letter(col)].width = 14

    # ========================================
    # Sheet 7: Sammanfattning
    # ========================================
    ws_summary = wb.create_sheet("Sammanfattning")

    ws_summary['A1'] = "RESULTATSAMMANFATTNING"
    ws_summary['A1'].font = Font(bold=True, size=14)
    ws_summary.merge_cells('A1:C1')

    # Key results section
    ws_summary['A3'] = "NYCKELRESULTAT"
    ws_summary['A3'].font = Font(bold=True)
    ws_summary['A3'].fill = header_fill
    ws_summary['A3'].font = header_font

    summary_items = [
        ("Total produktion", "=Parametrar!B16", "MWh", "0.0"),
        ("Baseload-behov", "=Parametrar!B17", "MWh", "0.0"),
        ("Leveransgrad", "=Parametrar!B16/Parametrar!B17", "%", "0.0%"),
        ("", "", "", ""),
        ("Total deficit", "=Parametrar!B18", "MWh", "0.00"),
        ("Total surplus", "=Parametrar!B19", "MWh", "0.00"),
        ("Energibalans", "=Parametrar!B19*Parametrar!B5-Parametrar!B18", "MWh", "0.00"),
        ("", "", "", ""),
        ("Deficit-timmar", "=Parametrar!B20", "st", "0"),
        ("Surplus-timmar", "=Parametrar!B21", "st", "0"),
        ("", "", "", ""),
        ("Max deficit", "=Parametrar!B22", "MW", "0.000"),
        ("Max batteribehov", "=Parametrar!B23", "MWh", "0.00"),
        ("Batteriduration", "=Parametrar!B24", "h", "0.0"),
    ]

    for i, (label, formula, unit, fmt) in enumerate(summary_items):
        row = 4 + i
    ...
    # Feasibility check
    ws_summary['A20'] = "FEASIBILITY"
    ws_summary['A20'].font = Font(bold=True)

    ws_summary['A21'] = "Energibalans OK?"
    ws_summary['B21'] = '=IF(B10>=0,"JA","NEJ - Omöjligt")'
    ...
    # ========================================
    wb.save(filepath)
    print(f"Validerings-Excel sparad: {filepath}")
    return filepath
